...
import { Sparkles, Zap, Clock, BarChart3, Copy, Wand2, Loader2, Check, AlertCircle, Code, Eye } from "lucide-react";
...
export function Dashboard() {
  ...
  const handleCastSpell = useCallback(async () => {
    ...
    try {
      ...
      if (store.lastGeneration) {
        console.log('Generated code received in Dashboard:');
        console.log(store.lastGeneration.code);
        console.log('Code length:', store.lastGeneration.code.length);
        console.log('First 500 chars:', store.lastGeneration.code.substring(0, 500));
      }
    } catch (error: any) {
  ...
  return (
    <div className="h-full p-8 overflow-y-auto">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Generated Code Display with Preview */}
        <AnimatePresence>
          {lastGeneration && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              ...
              <div className={`grid gap-6 ${showPreview ? 'lg:grid-cols-2' : 'grid-cols-1'}`}>
                ...
                <motion.div
                  layout
                  className="bg-zinc-900/80 backdrop-blur-xl rounded-2xl border border-emerald-500/20 overflow-hidden"
                >
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="bg-black/50 rounded-lg border border-emerald-500/20">
                      <div className="flex items-center justify-between px-4 py-2 border-b border-emerald-700/50 bg-emerald-900/20">
                        <span className="text-xs text-emerald-300 font-medium">Component.tsx</span>
                        <span className="text-xs text-zinc-500">TypeScript + React + Tailwind</span>
                      </div>
                      <pre className="p-4 text-sm text-emerald-300 whitespace-pre-wrap font-mono overflow-x-auto max-h-[600px]">
                  ...
                  >
                    <AdvancedPreview 
                      ...
                      onCodeUpdate={(newCode) => {
                        // Update the store with edited code
                        const store = useCrystalStore.getState();
                        store.setLastGeneration({
                          ...store.lastGeneration,
                          code: newCode
                        });
                      }}
                    />
